...
//判断是否安装过程序
if(is_file('../mdata/install.lock') && $_GET['step'] != 5){
...
<div class="header">
  <div class="layout">
    ...
    <div class="version">版本: v1.0 test</div>
  </div>
</div>
EOF;
...
function step3(&$install_error,&$install_recover){
    ...
    //管理员账号密码
	$mysqli->query("UPDATE {$db_prefix}admin SET id = '1' , name = '{$username}' ,password = '". md5($password) ."' ,outtime = '".time()."' ,roles = '1' ,create_time = '".time()."' , update_time = '".time()."' WHERE id = 1");
