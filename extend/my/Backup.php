...
//        }
    ...
    public function import($start)
    {
        ...
        if ($this->config['compress']) {
            $gz = gzopen($this->config['path'].$this->file['name'], 'r');
            $size = 0;
        } else {
    ...
    public function backup($table, $start)
    {
        ...
        if ($count) {
            ...
            foreach ($result as $row) {
                ...
                $rowsql = '';
                foreach($row as $v){
                  if(strpos($list[$sss]["Type"],'int') !== false || strpos($list[$sss]["Type"],'varchar') !== false){    
                  if($v === NULL){
                      $rowsql .= "NULL, ";
                  }else{
                      $rowsql .= "$v, ";
                ...
                $sql = "INSERT INTO `{$table}` VALUES (" . $rowsql . ");\n";
                if (false === $this->write($sql)) {
                    return false;
                }
            }
    ...
    public function optimize($tables = null)
    {
        if ($tables) {
            ...
            if ($list) {
                ...
                throw new \Exception("data sheet'{$tables}'Optimize mistakes please try again!");
            }
        } else {
            throw new \Exception("Please specify the table to be optimize!");
        }
    }
    /**
     * 分析表
     ...
     * @return String $tables  
     */
    public function analyze($tables = null)
    {
        if ($tables) {
            $db = self::connect();
            if (is_array($tables)) {
                $tables = implode('`,`', $tables);
                $list = $db->query("ANALYZE TABLE `{$tables}`");
            } else {
                $list = $db->query("ANALYZE TABLE `{$tables}`");
            }
            if ($list) {
                ...
                throw new \Exception("data sheet'{$tables}'Analyze mistakes please try again!");
            }
        } else {
            throw new \Exception("Please specify the table to be analyze!");
        }
    }
    /**
     * 修复表
